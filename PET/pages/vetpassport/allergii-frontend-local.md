...
Каждая неудалённая локальная аллергия переносится элементом `allergies` тела `POST /import/local-data` (см. «Импорт локальных данных — Backend», раздел о переносе Ведпаспорта).
